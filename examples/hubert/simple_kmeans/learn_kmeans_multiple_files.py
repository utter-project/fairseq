...
def load_feature_shard(feat_path, leng_path, percent):
...
def load_feature(feat_dir, percent):
    assert percent <= 1.0

    files = glob.glob(feat_dir + "/*.npy")

    feat = np.concatenate(
        [
            load_feature_shard(f_path, f_path.replace(".npy",".len"), percent)
            for f_path in files
        ],
        axis=0,
    )
...
def learn_kmeans(
    ...
    np.random.seed(seed)
    feat = load_feature(feat_dir, percent)
